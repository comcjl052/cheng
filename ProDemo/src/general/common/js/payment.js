;(function(window){
	...
	u.wxPay = function(ret){//微信支付
		if(ret.code != 0){
			...
			var is_decca = ret.data.is_decca; //台卡支付
			var order_sn = ret.data.order_sn;//台卡支付订单
			if(wx_mode == 1){
				WeixinJSBridge.invoke(
					...
					function(res){
						if(res.err_msg == "get_brand_wcpay_request:ok"){
							if(is_jfb == 1){
								Com.openWin("pay_success",{total_fee:total_fee});
							}else if(is_spell == 1){
								Com.openWin("group_detail",{open_sn:open_sn});
							}else if(is_decca == 1){
								Com.openWin("payment_success",{order_sn:order_sn});
							}else if(card_code){
