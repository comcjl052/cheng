...
class Radio extends React.Component{
    ...
    changeHandle(e) {
        if(this.props.s_radioType !="checkbox"){
            this.props.f_changeHandler && this.props.f_changeHandler(e.target.value);
        }else {
            this.props.f_changeHandler && this.props.f_changeHandler(e.target.checked);
        }
    }
    render(){
        let s_radioType = this.props.s_radioType || "radio";
        let s_name = this.props.s_name || "radio";
        let s_value = this.props.s_value;
        let s_id = "" + s_value + (new Date()).getTime();
        let e_input = null;
        if(this.props.b_default){
            e_input = <input id={s_id} ref="input" className="base-hide" onChange={(e)=>this.changeHandle(e)} type={s_radioType} name={s_name} defaultChecked value={s_value}/>;
        }else{
            e_input = <input id={s_id} ref="input" className="base-hide" onChange={(e)=>this.changeHandle(e)} type={s_radioType} name={s_name} value={s_value}/>;
        }
        return (
