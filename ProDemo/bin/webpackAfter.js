...
* @Last Modified by:   卢旺
* @Last Modified time: 2016-08-24 11:31:54
...
//拷贝不通过require引入的文件到html目录下
function copyFile(fromPath,toPath) {
    var data=fs.readFileSync(fromPath,"utf-8");
    fs.writeFileSync(toPath,data);
}
copyFile(getPath('src/base/js/config'+release_type+'.js'),getPath(outputDir+'/lib/config.js'));
copyFile(getPath('src/base/js/browser.js'),getPath(outputDir+'/lib/browser.js'));
copyFile(getPath('src/base/css/weixinBrowser.css'),getPath(outputDir+'/css/weixinBrowser.css'));
copyFile(getPath('src/base/css/wapBrowser.css'),getPath(outputDir+'/css/wapBrowser.css'));
//输出目录下所有js,css文件列表
...
//全局配置，用于替换diy部分css引入路径
var configData = require(getPath(outputDir + '/lib/config.js'));
function genHtmlFiles(path){
    walk(path,function(tmpPath){
        ...
        data = insStrIndex(data,'</title>',insConfigJs,fileName,1);
        var configPath = "";
        ...
        var patt = /http:\/\/.*?resource\/DIY\/css/g;
        var pattMatch = data.match(patt);
        if(pattMatch && pattMatch.length > 0){
            var pattRe = new RegExp(pattMatch[0],"g");
            data = data.replace(pattRe,configData.PC_ROOT_PATH+'/resource/DIY/css');
        }
